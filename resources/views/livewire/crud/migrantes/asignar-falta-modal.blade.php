<div>
    ...
    <label for="asignarFaltaModal-{{ $migranteId }}" class="btn btn-error text-base-content pr-5">
        <span class="icon-[ci--error] size-6"></span>
        Asignar Falta Disciplinaria
    </label>

    <!-- Put this part before </body> tag -->
    <input type="checkbox" id="asignarFaltaModal-{{ $migranteId }}" class="modal-toggle" />
    <div class="modal" role="dialog">
        <div class="modal-box max-w-5xl h-max w-2/3 bg-neutral border-2 border-accent flex flex-col">
            ...
            <p class="text-lg my-4 text-center w-full"><strong>Gravedad: </strong>{{ $gravedad }}</p>
            <div class="modal-action mt-2">
                <button class="btn btn-success text-base-content" wire:click="asignar">
                    Asignar
                </button>
                <label for="asignarFaltaModal-{{ $migranteId }}" class="btn btn-accent text-base-content">Cancelar</label>
            </div>
        </div>
    </div>
</div>
@script
    <script>
        $wire.on('close-modal', () => {
            ...
            document.getElementById('asignarFaltaModal-{{ $migranteId }}').checked = false;
        });
    </script>
@endscript
